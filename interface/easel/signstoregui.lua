...
function init()
  ...
  uniqueEaselIdentifier = "Easel@" .. world.entityPosition(console.sourceEntity())[1] ..  world.entityPosition(console.sourceEntity())[2]
  if world.getProperty(uniqueEaselIdentifier.."storage") ~= nil then
    storage = world.getProperty(uniqueEaselIdentifier.."storage")
    --world.logInfo("World Storage Found: %s", world.getProperty(uniqueEaselIdentifier.."storage"))
    templates = world.getProperty(uniqueEaselIdentifier.."templates")
    --world.logInfo("World Templates Found: %s", world.getProperty(uniqueEaselIdentifier.."templates"))
  else
    storage = {}
    templates = {}
  end
  ...
  dropSpot = world.entityPosition(console.sourceEntity())
  
  --storage.buttonStates["undoPress"] = "grayed" storage.buttonStates["pickToggle"] = "off" storage.buttonStates["fillToggle"] = "off"
  
  local matchingCabinetList = world.objectQuery({world.entityPosition(console.sourceEntity())[1]+7,world.entityPosition(console.sourceEntity())[2]},1)
  for i,j in ipairs(matchingCabinetList) do
    if world.entityName(j) == "signdispenser" then self.matchingCabinet = j end
  end
  if self.matchingCabinet == nil then console.dismiss() end
  ...
  world.sendEntityMessage(console.sourceEntity(), "storeData", storage, templates)
end

function update(dt)
  local toPointer = console.canvasMousePosition()
  ...
  if button ~= nil then
    ...
    elseif button == "pixelPress" then
      if self.currentCursorModifier == "pickPixel" then
        self.toolTip = "[Clique Gauche] pour la couleur"
      elseif self.currentCursorModifier == "fillPixel" then
        self.toolTip = "[Clique Gauche] pour remplir"
      else
        self.toolTip = "[Left Click] pour dessiner, [Clique Droit] pour effacer"
      end
      blankTip = false
...
function canvasClickEvent(position, button, isButtonDown)
  if self.thrownError == nil and self.inputBoxMode == nil then  --message boxes block input
    if isButtonDown and (button == 1 or button == 3) then     --middle mouse button isn't even a real mouse button man
      --world.logInfo("Click @ %s", position)
      ...
      if storage.oldPointer == nil then
        ...
        if storage.oldButton == "pixelPress" then
          if self.currentCursorModifier == nil then
            if button == 1 then --left click to draw
              storage.currentPixelValueTo = storage.paintColor
            elseif button == 3 then --right click to erase
    ...
    elseif isButtonDown == false and (button == 1 or button == 3) then
      self.clickplease = 0  
    end
  end
end
...
function drawSignCanvas()
  ...
  --Main Anchor--
  ---------------
   console.canvasDrawImage("/interface/easel/staticpanels.png", buttonPosition("S:mainStatics"), 1) 
   --console.canvasDrawImage("/interface/easel/icongrab.png:"..storage.buttonStates["iconGrab"],buttonPosition("iconGrab"), 1) 
   console.canvasDrawImage("/interface/easel/signbacking.png:"..storage.backingTypes[storage.backingTypesIndex], buttonPosition("pixelPress"), 1.5) 
   console.canvasDrawImage("/interface/easel/signframe.png:"..storage.frameTypes[storage.frameTypesIndex][1], buttonPosition("framePress"), 1.5) 
   console.canvasDrawImage("/interface/easel/minisignbacking.png:"..storage.backingTypes[storage.backingTypesIndex], buttonPosition("S:miniBacking"), 1) --(20,6.5)
   console.canvasDrawImage("/interface/easel/scrollleft.png:"..storage.buttonStates["backingLeft"], buttonPosition("backingLeft"), 1) 
   console.canvasDrawImage("/interface/easel/scrollright.png:"..storage.buttonStates["backingRight"], buttonPosition("backingRight"), 1) 
   console.canvasDrawImage("/interface/easel/scrollleft.png:"..storage.buttonStates["frameLeft"], buttonPosition("frameLeft"), 1) 
   console.canvasDrawImage("/interface/easel/scrollright.png:"..storage.buttonStates["frameRight"], buttonPosition("frameRight"), 1) 
   console.canvasDrawImage("/interface/easel/delete.png:"..storage.buttonStates["frameDelete"], buttonPosition("frameDelete"), 1) 
   console.canvasDrawImage("/interface/easel/add.png:"..storage.buttonStates["frameAdd"], buttonPosition("frameAdd"), 1) 
   console.canvasDrawImage("/interface/easel/shiftleftright.png:right"..storage.buttonStates["shiftRight"], buttonPosition("shiftRight"), 1) 
   console.canvasDrawImage("/interface/easel/shiftleftright.png:left"..storage.buttonStates["shiftLeft"], buttonPosition("shiftLeft"), 1) 
   console.canvasDrawImage("/interface/easel/shiftupdown.png:up"..storage.buttonStates["shiftUp"], buttonPosition("shiftUp"), 1) 
   console.canvasDrawImage("/interface/easel/shiftupdown.png:down"..storage.buttonStates["shiftDown"], buttonPosition("shiftDown"), 1) 
   console.canvasDrawImage("/interface/easel/wiredicon.png:"..storage.buttonStates["isWired"], buttonPosition("isWired"), 1) 
   ...
   if storage.lightData ~= nil then
     ...
     console.canvasDrawRect(button4Position("S:lightColor"), convertRGBAtoArray(storage.lightData))
     console.canvasDrawImage("/interface/easel/lightbutton.png:"..storage.buttonStates["lightSelect"], buttonPosition("lightSelect"), 1) 
   else
    console.canvasDrawImage("/interface/easel/lightbutton.png:"..storage.buttonStates["lightSelect"], buttonPosition("lightSelect"), 1) 
   end
  ...
  console.canvasDrawImage("/interface/easel/numbers.png:"..digit11,buttonPosition("S:digit11"), 1) 
  console.canvasDrawImage("/interface/easel/numbers.png:"..digit12, buttonPosition("S:digit12"), 1) 
  console.canvasDrawImage("/interface/easel/numbers.png:"..digit21,buttonPosition("S:digit21"), 1) 
  console.canvasDrawImage("/interface/easel/numbers.png:"..digit22, buttonPosition("S:digit22"), 1) --(-1,3)
  for x=1,32 do
    for y=1,8 do
       if storage.signPixels[storage.currentFrame][x][y] ~= 0 then
        console.canvasDrawRect(
          {buttonPosition("pixelPress")[1]-6+6*x, buttonPosition("pixelPress")[2]-6+6*y, buttonPosition("pixelPress")[1]+6*x, buttonPosition("pixelPress")[2]+6*y},
  ...
  --Template Anchor--
  -------------------  
  console.canvasDrawImage("/interface/easel/new.png:"..storage.buttonStates["dumpPress"], buttonPosition("dumpPress"), 1) --(4,8)
  console.canvasDrawImage("/interface/easel/clear.png:"..storage.buttonStates["clearPress"], buttonPosition("clearPress"), 1) --(4,8)
  console.canvasDrawImage("/interface/easel/print.png:"..storage.buttonStates["printPress"], buttonPosition("printPress"), 1) --(8,8)
  console.canvasDrawImage("/interface/easel/undo.png:"..storage.buttonStates["undoPress"], buttonPosition("undoPress"), 1) --(12,8)
   
  console.canvasDrawImage("/interface/easel/scaninput.png:"..storage.buttonStates["templateSlotScanButton"], buttonPosition("templateSlotScanButton"), 1)
  console.canvasDrawImage("/interface/easel/iconthing.png", buttonPosition("templateSlotCopy"), 1)
  if templates["fromContainer"] ~= nil then
    console.canvasDrawImage("/objects/outpost/customsign/"..templates["fromContainer"].currentIcon, vec2.add(buttonPosition("templateSlotCopy"), {1,0}), 1)
  end
  if storage.buttonStates["templateSlotCopy"] == "highlight" then 
    console.canvasDrawRect(button4Position("templateSlotCopy"), {255,255,255,70}) end

  console.canvasDrawText(self.toolTip, {position=buttonPosition("S:toolTip")}, 8, {210,210,210,210})
  
  ----------------
  --Color Anchor--
  ----------------
   console.canvasDrawImage("/interface/easel/staticcolor.png", buttonPosition("S:colorStatics"), 1) --(12,8)
   console.canvasDrawImage("/interface/easel/pickericon.png:"..storage.buttonStates["pickToggle"], buttonPosition("pickToggle"), 1) --(19.5,3)
   console.canvasDrawImage("/interface/easel/fillicon.png:"..storage.buttonStates["fillToggle"], buttonPosition("fillToggle"), 1) --(21,3)
   console.canvasDrawRect(button4Position("S:colorDisplay"),convertRGBAtoArray(storage.paintColor))
  ...
  ----------------  
  if self.thrownError ~= nil then
    console.canvasDrawImage("/interface/easel/errorpane.png", buttonPosition("S:errorWindow"), 1)
    console.canvasDrawText(self.thrownError[2], {position=buttonPosition("S:errorPrompt1")},20,{220,0,0,255})
    console.canvasDrawText(self.thrownError[3], {position=buttonPosition("S:errorPrompt2")},20,{220,0,0,255})
    if self.thrownError[1] == 1 then self.thrownError = nil else self.thrownError[1] = self.thrownError[1] - 1 end
  end
  
  if self.colorSelectBox then
    console.canvasDrawImage("/interface/easel/spectrumchart_window.png", buttonPosition("S:spectrumWindow"), 1)
    console.canvasDrawImage("/interface/easel/spectrumchart.png", buttonPosition("spectrumPress"), 1)
  end
  
  if self.showSpectrumCursor[1] > 0 then
    console.canvasDrawImage("/interface/easel/spectrumcursor.png", self.showSpectrumCursor[2], 1)
...
function doButton(button, position)
  ...
  elseif button == "undoPress" and #self.undoStack > 0 then --the undo button has to do, well, pretty much all that the other buttons do. just in reverse.
    ...
    --world.logInfo("Undoing %s", action)
...
function assignInputString(stringType, input)
  ...
  if isInvalid then
    self.thrownError = {300, "ERREUR! ARGUMENTS INVALIDE", "OBEY GIVEN CRITERIA"}
  end
end
...
function requestSignDrop(args)
  world.sendEntityMessage(console.sourceEntity(), "requestSignDrop", dropSpot)
...
function lookForTemplateSign()
  ...
  if self.droppedSign ~= nil then
    ...
    --world.logInfo("Dropped sign: %s", data.droppedSign)
  ...
  for i=1,#directiveStrings do 
    ...
    if directiveStrings[i] ~= nil and directiveStrings[i] ~= "" then
      ...
      while (p + 16 <= #directiveStrings[i]) do
        ...
        --world.logInfo("Frame %s, Pixel %s,%s", i, pix, piy)
...
function outputSign()
  --world.logInfo("//////////Printing Sign//////////")
    local inNodesToAdd = nil local outNodesToAdd = nil
  if storage.isWired then
    inNodesToAdd = {{-2,0}}
    outNodesToAdd = {{1,0}}
  else
  ...
  parameters.inboundNodes = inNodesToAdd
  parameters.outboundNodes = outNodesToAdd
  ...
  --world.logInfo("Wired: %s - Container: %s - Backing: %s - Frame: %s", storage.isWired, storage.isContainer, storage.backingTypes[storage.backingTypesIndex], storage.frameTypes[storage.frameTypesIndex][2])
  --world.logInfo("%s", storage.signDirectiveStrings")
  local signToPlace = {}
  ...
  --world.logInfo("Sign To Place: %s", signToPlace)
  ...
  if #fitSpots > 0 and fitSpots[1] ~= 8 then
    world.sendEntityMessage(console.sourceEntity(), "putInChest", signToPlace, fitSpots[1])
    outputSuccess = true
  else
    for i = 0, 7 do
      if not world.containerItemAt(self.matchingCabinet, i) then
        world.sendEntityMessage(console.sourceEntity(), "putInChest", signToPlace, i)
        outputSuccess = true
        break
      end
    end
...
function buttonPosition(button)
  if canvasButtonOffsets[button] == nil then world.logInfo("%s", button) end
...
function recursiveFiller(p, oc)
  ...
  --world.logInfo("Filling at position %s: %s to %s", p, storage.paintColor, oc)
